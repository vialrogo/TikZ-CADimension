...
\tikzset{%
    Cote node/.style={%
        %sloped,    % This option ratate the text to the line (horizontal, vertical)
        fill=white,
        font=\footnotesize, 
        inner sep=1pt, 
        outer sep=1.5pt,
...
\NewDocumentCommand{\Cote}{%
    ...
    \fi\fi\fi\fi
      ...
      \if #7r
      \else
        \draw[Cote limite, shorten >= #2, shorten <= -2*#2] (@4) -- #4 ;
        \draw[Cote limite, shorten >= #2, shorten <= -2*#2] (@5) -- #5 ;
      \fi

    \IfBooleanTF #1 {% Con asterisco
      \draw[Cote arrow,-] (@4) -- (@5) node[Cote node] at ($ 0.5*(@4) + 0.5*(@5) $) {#6\strut};
    ...
    \else
      \if #7r
        \draw[Cote radius] (@4) -- (@5) node[Cote node] at ($ 0.5*(@4) + 0.5*(@5) $) {#6\strut};
      \else
        \draw[Cote arrow] (@4) -- (@5) node[Cote node] at ($ 0.5*(@4) + 0.5*(@5) $) {#6\strut};
      \fi
    \fi
